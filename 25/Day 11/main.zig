...
pub fn main() !void {
    ...
    while (true) {
        ...
        while (itr.next()) |node| {
            ...
            try graph.getPtr(pidx).?.append(ga, nidx);
        }
    }
...
fn getOut(u: u12, e: u12, graph: *std.AutoHashMap(u12, AL(u12))) u32 {
    if (u == e) return 1;

    var ret: u32 = 0;
    for (graph.getPtr(u).?.items) |v| {
